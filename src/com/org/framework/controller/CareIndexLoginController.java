...
public class CareIndexLoginController {
	@Autowired
	UtilMethodService utilMethodService;
	public static Logger logger = Logger.getLogger(CareIndexLoginController.class);
	...
	public void Verification(HttpServletRequest request, HttpServletResponse response) throws ClientProtocolException, IOException {
		...
		System.out.println("短信返回的信息：" + back_msg);
		...
		logger.info("session中得信息"+msg_json);
		ResponseUtils.setToHttp(response, jsonback.toString());
	}
	...
	public void login(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		logger.info("手机号为："+phone);
		logger.info("验证码为："+verCode);
...
/*		if (verCode.isEmpty()) {
		...
		if ((System.currentTimeMillis() - json.getLong("createTime")) > 1000 * 60 * 5) {
			...
			ResponseUtils.setToHttp(response, jsonback.toString());
			logger.info("验证码时间信息"+jsonback.toString());
			return;
		}
		...
		String sql = "insert into wx_phone_for_report(user_phone,create_time) values(?,?)";
	...
	@RequestMapping("/check_phone.hn")
	public void check_phone(HttpServletRequest request, HttpServletResponse response) throws Exception {
		logger.info("大健康手机号登录方法start");
		JSONObject jsonback = new JSONObject();
		String phone = request.getParameter("phone") == null ? "" : request.getParameter("phone").trim();
		//JSONObject msg_json = new JSONObject();
		jsonback.put("phone", phone);
		request.getSession().setAttribute("phone", jsonback);// 存到session
		logger.info("session中得信息"+jsonback);
		jsonback.put("success", true);
		jsonback.put("msg", "校验手机号");
		ResponseUtils.setToHttp(response, jsonback.toString());
//		String verCode = request.getParameter("verCode") == null ? "" : request.getParameter("verCode").trim();
		JSONObject json = JSONObject.fromObject(request.getSession().getAttribute("phone"));
		System.out.println("获取的session内容：" + json);
...
/*
		...
		logger.info("大健康校验手机号方法end"+jsonback.toString());
	}
